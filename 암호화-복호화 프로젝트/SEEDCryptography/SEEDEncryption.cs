...
namespace SEEDCryptography
{
    [EventTrackingEnabled(true)]
    [Description("Interface Serviced Component")]
    public class SEEDEncryption
    {
        #region SSN
        ...
        {
            return SEEDEncryption.Encrypt(Data, seedKey, true);
        }

        public static byte[] Encrypt(byte[] Data, byte[] seedKey, bool cbcPad) //cbcPad는 항상 true로 설정할 것.
        {
            int nOutSize = (int)((Data.Length) / 16) * 16;
            //여기서 문제가 있음. 2009년 devpia(데브피아)에서 배포한 소스코드는 밑에 else문이 없으며, 이의 경우 16글자,32글자,48글자 단위의 평서문만 입력할 수 있었음.
            //이 소스코드는 2012년에 수정되었으며, else문이 추가로 나왔지만, cbcPad가 false일 경우 평서문이 16글자 이상일 때 nOutSize가 위의 코드에 의해 16*16으로 배가 되어버리는 이상이 있음.
            //따라서 cbcPad를 항상 true로 설정하는 것을 권장한다. (cbc모드)
            if (cbcPad)
            {
